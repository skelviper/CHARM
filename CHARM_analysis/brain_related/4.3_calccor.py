...
def calccor(tss_id):
    ...
    try:
        with warnings.catch_warnings():
            ...
            result.to_csv(f"./cor_res/{tss_id}.tsv.gz",sep="\t",index=False)
            return None
    except Exception as e:
        print(f"Error processing {tss_id}: {e}")
        return None

with concurrent.futures.ProcessPoolExecutor(240,mp_context=multiprocessing.get_context("fork")) as executor:
     list(tqdm.tqdm(executor.map(calccor, tss["tss_id"].sample(frac=1)), total=tss.shape[0]))

#calccor(tss_id="Satb2_1")

# with concurrent.futures.ProcessPoolExecutor(50,mp_context=multiprocessing.get_context("fork")) as executor:

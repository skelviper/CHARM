...
function _hic_resolve_frag(opt, a)
{
	...
	for (var i = 0; i < a.length; ++i) {
		...
		var read_origin = (flag & 0x40) ? 'R1' : ((flag & 0x80) ? 'R2' : '.');
		b.push([read_num, rev, qlen, qs, qe, t[2], rs, re, mapq, phase, read_origin]);  
	...
	var segs = [[b[0][5], b[0][6], b[0][7], b[0][1] ? '-' : '+', b[0][9], b[0][8], 1, b[0][10]]];
	for (var i = 1; i < b.length; ++i) {
		...
		if (new_seg) {
			...
			segs.push([q[5], q[6], q[7], q[1] ? '-' : '+', q[9], q[8], 1, q[10]]);
		} else {

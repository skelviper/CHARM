...
find ./Rawdata/ -name "*R1.fq.gz" | parallel --tag 'zcat {} | wc -l' | sort > ./stat/raw.fq.stat
...
for i in `find ./result/cleaned_pairs/c12 -name "*.pairs.gz"|sort`; do echo -n $i;echo -n -e "\t";zcat $i | awk '$2!=$4 {print $0}' | wc -l; done > ./stat/inter.pairs.c12.stat
